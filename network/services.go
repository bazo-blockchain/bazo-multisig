...
func checkHealthService() {
	for {
		time.Sleep(utils.HEALTH_CHECK_INTERVAL * time.Second)

		//Stop trying to connect after #numberOfRetry attempts.
		numberOfRetry := 0

		if !peers.contains(utils.Config.BootstrapIpport) {
			p, err := initiateNewClientConnection(utils.Config.BootstrapIpport)
			if p == nil || err != nil {
				logger.Printf("%v\n", err)
			} else {
				go minerConn(p)
		...
		if len(peers.minerConns) >= utils.MIN_MINERS {
			continue
		}
