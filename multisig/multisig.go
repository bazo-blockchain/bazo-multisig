...
func listener() {
	listener, err := net.Listen("tcp", ":"+strings.Split(utils.Config.ThisIpport, ":")[1])
	if err != nil {
		logger.Fatal(err)
	}
...
func serve(c net.Conn) {
	...
	if header.TypeID == p2p.FUNDSTX_BRDCST {
		...
		for _, data := range protocol.Decode(payload, protocol.FUNDSTX_SIZE) {
			...
			delete(network.NonVerifiedTxs, verifiedTx.Hash())
		}
		...
		for _, tx := range network.NonVerifiedTxs {
...
func processTx(tx *protocol.FundsTx) (err error) {
	...
	if err := verify(tx, acc); err == nil {
		...
		for _, nonVerifiedTx := range network.NonVerifiedTxs {
			if nonVerifiedTx.To == tx.To && nonVerifiedTx.Sig2 == [64]byte{} {
				delete(network.NonVerifiedTxs, nonVerifiedTx.Hash())
			}
		}

		network.NonVerifiedTxs[tx.Hash()] = tx
...
func reqAccount(addressHash [32]byte) (acc *client.Account, err error) {
	response, err := http.Get("http://" + utils.Config.ClientIpport + "/account/" + hex.EncodeToString(addressHash[:]))
	if err != nil {
...
func sendTx(tx *protocol.FundsTx) error {
	...
	txHash := tx.Hash()
	response, err := http.Post("http://"+utils.Config.ClientIpport+"/sendFundsTx/"+hex.EncodeToString(txHash[:])+"/"+hex.EncodeToString(tx.Sig2[:]), "application/json", bytes.NewBuffer(jsonValue))
	if err != nil {
